...
void RayCastingRenderer::set2DTransferFunction( float* tfunc2d, size_t resolution_x, size_t resolution_y )
{
    m_2d_tfunc_data = tfunc2d;
    m_2d_width = resolution_x;
    m_2d_height = resolution_y;
}    
  
// add another volume    
void RayCastingRenderer::addVolume( kvs::StructuredVolumeObject* volume )
{
    m_volume_2 = volume;
}
...
//        std::cout<< temp << std::endl;
        ...
        // Ray casting.
        m_ray_caster.bind();
        glActiveTexture( GL_TEXTURE4 ); m_2d_transfer_function_texture.bind(); glEnable( GL_TEXTURE_2D );
        ::CheckOpenGLError( "5 error." );
        ...
        ::CheckOpenGLError( "7 error." );

        glActiveTexture( GL_TEXTURE8 ); m_volume_data_2.bind();
        ::CheckOpenGLError( "6 error." );
...

        {
            ...
            ::CheckOpenGLError( "2 error." );

            this->draw_quad( 1.0f );
        }
        glActiveTexture( GL_TEXTURE4 ); m_2d_transfer_function_texture.unbind(); glDisable( GL_TEXTURE_2D );
...
//        m_ray_caster.setUniformValuef( "volume.max_value", max_value );
        std::cout << "tfunc_width: " << m_2d_width << std::endl;
        std::cout << "tfunc_height: " << m_2d_height << std::endl;
        m_ray_caster.setUniformValuef( "tfunc_width", m_2d_width );
        m_ray_caster.setUniformValuef( "tfunc_height", m_2d_height );
...
void RayCastingRenderer::create_2d_transfer_function( void )
{    
    m_2d_transfer_function_texture.setWrapS( GL_CLAMP_TO_EDGE );
    m_2d_transfer_function_texture.setWrapT( GL_CLAMP_TO_EDGE );
    m_2d_transfer_function_texture.setMagFilter( GL_LINEAR );
    m_2d_transfer_function_texture.setMinFilter( GL_LINEAR );
    m_2d_transfer_function_texture.setPixelFormat( GL_RGBA32F_ARB, GL_RGBA, GL_FLOAT );
    m_2d_transfer_function_texture.create( m_2d_width, m_2d_height );
    m_2d_transfer_function_texture.download( m_2d_width, m_2d_height, m_2d_tfunc_data );
    
    ::CheckOpenGLError( "Cannot create 2d transfer function texture." );
}
...
// create the texture for the 2nd volume 
void RayCastingRenderer::create_volume_data_2( const kvs::StructuredVolumeObject* volume )
{
    const size_t width = volume->resolution().x();
    const size_t height = volume->resolution().y();
    const size_t depth = volume->resolution().z();
    
    m_volume_data_2.release();
    m_volume_data_2.setWrapS( GL_CLAMP_TO_BORDER );
    m_volume_data_2.setWrapT( GL_CLAMP_TO_BORDER );
    m_volume_data_2.setWrapR( GL_CLAMP_TO_BORDER );
    m_volume_data_2.setMagFilter( GL_LINEAR );
    m_volume_data_2.setMinFilter( GL_LINEAR );
    ...
    else if ( type == typeid( kvs::Real32 ) )
    {
        ...
        data_value = ::NormalizeValues<kvs::Real32>( volume, min_value, max_value );
        std::cout << "The type is float in create volume for y" << std::endl;
        std::cout << "min: " << min_value << std::endl;
        std::cout << "max: " << max_value << std::endl;
    }
    else if ( type == typeid( kvs::Real64 ) )
    ...
    m_volume_data_2.setPixelFormat( data_format, GL_ALPHA, data_type );
    m_volume_data_2.create( width, height, depth );
    m_volume_data_2.download( width, height, depth, data_value.data() );
    m_volume_data_2.unbind();
    
    ::CheckOpenGLError( "Cannot create the 2nd volume data texture." );
}
